...
import com.joker.demo.netty.domain.*;
import com.joker.demo.netty.util.CacheUtil;
import com.joker.demo.netty.util.FileUtil;
            ...
            case 0: // 0传输文件'请求'
                ...
                if (null != fileBurstInstructOld) {
                    ...
                    System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
                            + " joker 服务端，接收客户端传输文件请求[断点续传]。" + JSON.toJSONString(fileBurstInstructOld));
                ...
                System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
                        + " joker 服务端，接收客户端传输文件请求。" + JSON.toJSONString(fileDescInfo));
                break;
            case 2: // 2文件传输'数据'
                FileBurstData fileBurstData = (FileBurstData) fileTransferProtocol.getTransferObj();
                FileBurstInstruct fileBurstInstruct = FileUtil.writeFile("F:\\03-code\\01_mySourceCode\\joker-demo-netty\\joker-demo-netty-2-04\\src\\main\\resources", fileBurstData);
                ...
                System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
                        + " joker 服务端，接收客户端传输文件数据。" + JSON.toJSONString(fileBurstData));
